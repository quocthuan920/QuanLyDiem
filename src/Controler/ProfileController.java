...
public class ProfileController {
    ...
    public void setEven() {
        try {
            ...
            if (rs.next()) {
                String ten = rs.getString("hotensv"); //lấy tên
                jLbTen.setText(ten);
                ...
                String ngaysinh = rs.getString("ngaysinh");//lấy ngày sinh
                jLbNgaySinh.setText(ngaysinh);
                String nganh = rs.getString("tennganh"); // lấy tên ngành
                jLbNganh.setText(nganh);
                String lop = rs.getString("tenlop");     // lấy tên lớp sinh hoạt
                jLbLop.setText(lop);
                if (rs.getInt("totnghiep") == 1) {
                    jlbTrangThai.setText("Đã tốt nghiệp");  // đã tốt nghiệp hay chưa???
                    
                    BoTinhLOi loi=new BoTinhLOi();
                    double[] value=loi.tiledatloi(mssv);    // gọi class LO để tính LO[i] 
//                    for (int i=0;i<11;i++){
...
//                    }
                    ...
                    jpg9.setVisible(true);               // hiển thị ra panel
