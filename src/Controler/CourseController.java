...
import View.Sinhvien.CoursePanel;
    ...
    public void setEvent() {
        try {
            ...
            while (rs.next()) {
                ...
                vt.add("");
                model.addRow(vt);
                jTTile.setModel(model); //nhập các giá trị vào bảng bằng Vector
            }
            ps.close();
            rs.close();
            BoTinhGi ctrl = new BoTinhGi(); // gọi class BoTinhGi để tính G[i]
            for (int i = 0; i < jTTile.getRowCount(); i++) {
                ...
                if (!rs.next()) {
                    ...
                    double[] tile = new double[4]; //tạo mảng để chứa kết quả tỉ lệ G[i]
                    tile = ctrl.getTiLeDatGi(mssv, jTTile.getValueAt(i, 0).toString()); // tính G[i] trả về biến tile
